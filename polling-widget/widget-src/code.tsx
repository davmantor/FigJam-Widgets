...
function PollingWidget() {
  ...
  const handleRemoveTextField = (index: number) => {
    // Create new arrays based on current state
    const updatedTextArray = [...textArray];
    const updatedVoteArray = [...voteArray];
    
    // Remove the text field at the given index
    updatedTextArray.splice(index, 1);
    
    // Remove the votes corresponding to the removed text field
    updatedVoteArray.splice(index, 1);
  ...
  return (
    ...
    >
      ...
      {textArray.map((item, index) => (
        <AutoLayout
          key={index}  // Ensure proper rendering and updating
