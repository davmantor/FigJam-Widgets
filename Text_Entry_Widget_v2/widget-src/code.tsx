...
function Widget() {
  ...
  useEffect(() => {
    ...
    figma.ui.onmessage = (msg) => {
      ...
      if (msg.type === 'addResponse') {
        setPreviousResponses(prev => [msg.response, ...prev]);
      }
      if (msg.type === 'revealPrevious') {
        setShowPrevious(msg.showPrevious);
      }
      if (msg.type === 'resize') {
        setWidth(msg.width);
      ...
      if (msg.type === 'updateResponse') {
        setPreviousResponses(msg.ret);
      }
      if (msg.type === 'editCurrent') {
        setPreviousResponses(msg.ret);
      }
      if (msg.type === 'deleteResponse') {
        console.log("received deleteResponse", msg);
        setPreviousResponses(msg.ret);
      }
    }
    });
  ...
  const resetResponse = async (response: string, currentWidgetId: string|null=null) => {
    ...
    if (self) {
      // Store current response in previousResponses if it exists
  ...
  function deleteResponse(index) {
    ...
    console.log('Before deletion - Previous responses:', previousResponses);
    console.log('Attempting to delete response at index:', index);
    
    // Update local state
    previousResponses.splice(index, 1);
    
    // Log state after deletion
    console.log('After deletion - Previous responses:', previousResponses);
    
    // Send message to widget to update state with the new array
    parent.postMessage({ 
      pluginMessage: { 
        type: 'deleteResponse',
        index,
        previousResponses: previousResponses  // Send the updated array
      } 
    }, '*');
  ...
  function updateResponse(index, newResponse, newUserName, newPhotoUrl=undefined) {
    ...
    // Update local state
    params.previousResponses[index] = {
      ...params.previousResponses[index],
    ...
    // Send message to widget to update state
    parent.postMessage({ 
      pluginMessage: { 
        type: 'updateResponse',
        index,
